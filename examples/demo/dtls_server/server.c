...
static int connect_to_hsm_server(void)
{
    ...
    printf("Connecting to wolfHSM server via shared memory DMA...\n");
    ...
    printf("Registered WH_DEV_ID (0x%08X) crypto callback\n", WH_DEV_ID);
    ...
    printf("Registered WH_DEV_ID_DMA (0x%08X) crypto callback\n", WH_DEV_ID_DMA);
#endif
...
static void disconnect_from_hsm_server(void)
{
    ...
    wh_Client_CommClose(g_client);
    wh_Client_Cleanup(g_client);
    printf("Disconnected from wolfHSM server\n");
}
...
int main(int argc, char** argv)
{
    ...
    printf("===========================================\n");
    printf("wolfHSM Server Demo (DTLS mode)\n");
    printf("===========================================\n");
    printf("\n");
    printf("This server offloads crypto operations to\n");
    printf("the wolfHSM server running with --type dma\n");
    printf("\n");

    if (config.caCertFile) {
        printf("CA certificate: %s\n", config.caCertFile);
    } else {
        printf("CA certificate: (using built-in test certificate)\n");
    }
    printf("Port: %d\n", config.port);
    printf("\n");
    ...
    printf("All crypto operations were performed by wolfHSM\n");
    printf("===========================================\n\n");

    /* Simple echo loop - read and write back data */
    printf("Entering echo mode. Send data to the server...\n");
    while (1) {
