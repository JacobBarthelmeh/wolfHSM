 ...
 *
 * By default, uses DTLS 1.2 server method. This can be changed to TLS
 * by modifying the method call below.
 *
...
static int initialize_wolfssl(WOLFSSL_CTX** ctx, whClientContext* hsm_client)
{
    ...
    /* Create a new context for server - using DTLS 1.2 by default */
    *ctx = wolfSSL_CTX_new(wolfDTLSv1_2_server_method());
     ...
     *
     * If file paths are provided in config, load from files.
     * Otherwise, use the built-in test certificate buffers.
     */
    ...
    if (g_config.caCertFile != NULL) {
        ...
        printf("Loaded CA certificate from file: %s\n", g_config.caCertFile);
    }
    else {
        ...
        printf("Loaded built-in CA certificate for client verification\n");
    }
    ...
    if (g_config.serverCertFile != NULL) {
        ...
        printf("Loaded server certificate from file: %s\n", g_config.serverCertFile);
    }
    else {
        ...
        printf("Loaded built-in server certificate\n");
    }
    ...
    if (g_config.serverKeyFile != NULL) {
        ...
        printf("Loaded server private key from file: %s\n", g_config.serverKeyFile);
    }
    else {
        ...
        printf("Loaded built-in server private key\n");
    }
    ...
    wolfSSL_CTX_set_verify(*ctx,
                           ...
                           NULL);
    printf("Configured mutual authentication\n");
...
static int setup_ssl_accept(SERVER_CONTEXT* ctx)
{
    ...
    /*
     * Now that wolfSSL_new() succeeded (RNG is initialized with local software),
     * set the device ID on the SSL object so that crypto operations during
     * the handshake (RSA/ECC sign, key exchange) go through the wolfHSM server.
     */
    wolfSSL_SetDevId(ctx->ssl, WH_DEV_ID);
    printf("Set WH_DEV_ID (0x%08X) on SSL object for crypto offload\n", WH_DEV_ID);
                ...
                const ServerConfig* config)
{
    ...
    if (ret != 0) {
        printf("Failed to set up SSL accept\n");
        close(ctx->listenfd);
        wolfSSL_CTX_free(g_ctx);
        g_ctx = NULL;
        return -1;
